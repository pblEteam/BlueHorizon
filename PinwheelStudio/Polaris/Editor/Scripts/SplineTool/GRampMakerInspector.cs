...
namespace Pinwheel.Griffin.SplineTool
{
    ...
    public class GRampMakerInspector : Editor
    {
        ...
        private class GSplineMeshGUI
        {
            public static readonly string LABEL = "Mesh";
            public static readonly string ID = "spline-ramp-maker-mesh";

            public static readonly GUIContent CURVINESS = new GUIContent("Curviness", "High value produces smoother curves and vice versa");
            public static readonly GUIContent WIDTH = new GUIContent("Width", "Width of the inner part where it follow the spline path");
            public static readonly GUIContent FALLOFF_WIDTH = new GUIContent("Falloff Width", "Width of the outer part where it gradually blend with the terrain");
        }

        private void DrawSplineMeshGUI()
        {
            GEditorCommon.Foldout(GSplineMeshGUI.LABEL, false, GSplineMeshGUI.ID, () =>
            {
                instance.Curviness = EditorGUILayout.DelayedIntField(GSplineMeshGUI.CURVINESS, instance.Curviness);
                instance.Width = EditorGUILayout.DelayedFloatField(GSplineMeshGUI.WIDTH, instance.Width);
                instance.FalloffWidth = EditorGUILayout.DelayedFloatField(GSplineMeshGUI.FALLOFF_WIDTH, instance.FalloffWidth);
            });
        }
        ...
        private void DrawActionGUI()
        {
            ...
            GEditorCommon.Foldout(GActionGUI.LABEL, true, GActionGUI.ID, () =>
            {
                ...
                {
                    List<GStylizedTerrain> terrains = GUtilities.ExtractTerrainsFromOverlapTest(instance.SplineCreator.SweepTest(instance.Curviness, instance.Width, instance.FalloffWidth));
        ...
        {
            List<GOverlapTestResult> sweepTests = instance.SplineCreator.SweepTest(instance.Curviness, instance.Width, instance.FalloffWidth);
        ...
        private void DuringSceneGUI(SceneView sv)
        {
            ...
            //int vCount = instance.SplineCreator.GetVerticesCount(instance.Smoothness, instance.Width, instance.FalloffWidth);
            ...
            //instance.SplineCreator.GenerateWorldVerticesAndAlphas(splineVertices, splineAlphas, instance.Smoothness, instance.Width, instance.FalloffWidth);

            //Handles.color = Color.cyan;
            //Handles.EndGUI();
            //int triangleCount = splineVertices.Length / 3;
            //for (int i = 0; i < triangleCount; ++i)
            //{
            //    Vector3 v0 = splineVertices[i * 3 + 0];
            //    Vector3 v1 = splineVertices[i * 3 + 1];
            //    Vector3 v2 = splineVertices[i * 3 + 2];
