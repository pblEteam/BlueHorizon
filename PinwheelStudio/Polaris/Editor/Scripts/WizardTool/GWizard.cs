...
namespace Pinwheel.Griffin.Wizard
{
    public static class GWizard
    {
...
#if UNITY_EDITOR
                        string[] sampleSplatsGuid = AssetDatabase.FindAssets("l:PolarisSampleSplats");
                        if (sampleSplatsGuid != null && sampleSplatsGuid.Length > 0)
                        {
                            GSplatPrototypeGroup splats = AssetDatabase.LoadAssetAtPath<GSplatPrototypeGroup>(AssetDatabase.GUIDToAssetPath(sampleSplatsGuid[0]));
                            data.Shading.Splats = splats;
                        }
#endif

                        if (settings.texturingModel == GTexturingModel.GradientLookup)
                        {
                            data.Shading.UpdateLookupTextures();
                            data.Shading.SplatControlResolution = 32;
                        }
                        if (settings.texturingModel == GTexturingModel.ColorMap ||
                            settings.texturingModel == GTexturingModel.VertexColor)
                        {
                            data.Shading.SplatControlResolution = 32;
                        }
...
#if UNITY_EDITOR
                        string[] sampleTreesGuid = AssetDatabase.FindAssets("l:PolarisSampleTrees");
                        if (sampleTreesGuid != null && sampleTreesGuid.Length > 0)
                        {
                            GTreePrototypeGroup trees = AssetDatabase.LoadAssetAtPath<GTreePrototypeGroup>(AssetDatabase.GUIDToAssetPath(sampleTreesGuid[0]));
                            data.Foliage.Trees = trees;
                        }

                        string[] sampleGrassesGuid = AssetDatabase.FindAssets("l:PolarisSampleGrasses");
                        if (sampleGrassesGuid != null && sampleGrassesGuid.Length > 0)
                        {
                            GGrassPrototypeGroup grasses = AssetDatabase.LoadAssetAtPath<GGrassPrototypeGroup>(AssetDatabase.GUIDToAssetPath(sampleGrassesGuid[0]));
                            data.Foliage.Grasses = grasses;
                        }
#endif
        ...
        public static GNavigationHelper CreateNavHelper()
        {
            GameObject g = new GameObject("Nav Mesh Helper");
            g.transform.position = Vector3.zero;
            g.transform.rotation = Quaternion.identity;
            g.transform.localScale = Vector3.one;

            GNavigationHelper wind = g.AddComponent<GNavigationHelper>();
            return wind;
        }
