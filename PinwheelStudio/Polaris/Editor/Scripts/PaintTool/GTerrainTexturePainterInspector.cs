...
namespace Pinwheel.Griffin.PaintTool
{
    ...
    public class GTerrainPainterInspector : Editor
    {
        ...
        private void DrawSplatGUI()
        {
            ...
            {
                //GEditorSettings.Instance.paintTools.enableSceneViewRadialSelector = EditorGUILayout.ToggleLeft(GSplatGUI.SHORTCUT_HINT, GEditorSettings.Instance.paintTools.enableSceneViewRadialSelector, GEditorCommon.ItalicLabel);
                ...
                EditorGUILayout.LabelField("Assign a Splat Prototype Group to the terrain to display splat textures here.", GEditorCommon.WordWrapItalicLabel);
                GEditorCommon.DrawLinks("Learn more", "https://docs.google.com/document/d/1rbHv8CW_QoHdCEaj98xq_V4S_gdWbbmY9-kQGM8qnpM/edit#heading=h.ub8ldu9so77");
            });
        }
        ...
        private class GBrushGUI
        {
            ...
            public static readonly GUIContent SHORTCUT_HINT = new GUIContent("Press A/S/D/C in Scene View to adjust Radius/Rotation/Opacity/Scatter.");
        ...
        private void DrawBrushGUI()
        {
            ...
            {
                GEditorSettings.Instance.paintTools.enableSceneViewRadialSelector = EditorGUILayout.ToggleLeft(GBrushGUI.SHORTCUT_HINT, GEditorSettings.Instance.paintTools.enableSceneViewRadialSelector, GEditorCommon.ItalicLabel);
                GEditorCommon.Separator();
        ...
        {
            HandlePaintModeSelector();
            HandleBrushAdjusters();
        ...
        bool showRadialSelector = false;
        Vector2 radialSelectorCenter = Vector2.zero;
        GTexturePaintModeRadialSelectorProvider radialSelectorProvider = new GTexturePaintModeRadialSelectorProvider();

        private void HandlePaintModeSelector()
        {
            if (!GEditorSettings.Instance.paintTools.enableSceneViewRadialSelector)
                return;

            if (Event.current.type == EventType.KeyDown && Event.current.keyCode == KeyCode.V)
            {
                if (!showRadialSelector)
                {
                    radialSelectorCenter = Event.current.mousePosition;
                }
                showRadialSelector = true;
            }
            if (Event.current.type == EventType.KeyUp && Event.current.keyCode == KeyCode.V)
            {
                showRadialSelector = false;
            }

            if (showRadialSelector)
            {
                painter.Mode = GEditorCommon.RadialSelectorEnum<GTexturePaintingMode>(painter.Mode, radialSelectorCenter, radialSelectorProvider, GEditorCommon.DefaultRadialSelectorStyle);
            }
        }

        bool showRadiusAdjuster = false;
        bool showRotationAdjuster = false;
        bool showOpacityAdjuster = false;
        bool showScatterAdjuster = false;
        bool showColorSplatAdjuster = false;
        Vector2 brushAdjusterCenter = Vector2.zero;

        private void HandleBrushAdjusters()
        {
            if (!GEditorSettings.Instance.paintTools.enableSceneViewRadialSelector)
                return;
            ...
            if (Event.current.type == EventType.KeyDown)
            {
                if (Event.current.keyCode == KeyCode.A ||
                    ...
                    Event.current.keyCode == KeyCode.D ||
                    Event.current.keyCode == KeyCode.C /*||
                    Event.current.keyCode == KeyCode.V*/)
                {
                    if (!showRadiusAdjuster &&
                        !showRotationAdjuster &&
                        !showOpacityAdjuster &&
                        !showScatterAdjuster &&
                        !showColorSplatAdjuster)
                    {
                        brushAdjusterCenter = Event.current.mousePosition;
                    }
                }

                if (Event.current.keyCode == KeyCode.A)
                {
                    showRadiusAdjuster = true;
                }
                if (Event.current.keyCode == KeyCode.S)
                {
                    showRotationAdjuster = true;
                }
                if (Event.current.keyCode == KeyCode.D)
                {
                    showOpacityAdjuster = true;
                }
                if (Event.current.keyCode == KeyCode.C)
                {
                    showScatterAdjuster = true;
                }
            }
            if (Event.current.type == EventType.KeyUp)
            {
                if (Event.current.keyCode == KeyCode.A)
                {
                    showRadiusAdjuster = false;
                }
                if (Event.current.keyCode == KeyCode.S)
                {
                    showRotationAdjuster = false;
                }
                if (Event.current.keyCode == KeyCode.D)
                {
                    showOpacityAdjuster = false;
                }
                if (Event.current.keyCode == KeyCode.C)
                {
                    showScatterAdjuster = false;
                }
            }

            if (showRadiusAdjuster)
            {
                painter.BrushRadius = GEditorCommon.RadialFloatField("Radius", painter.BrushRadius, brushAdjusterCenter, GEditorCommon.DefaultRadialFloatFieldStyle, 0.5f);
            }
            if (showRotationAdjuster)
            {
                painter.BrushRotation = GEditorCommon.RadialFloatField("Rotation", painter.BrushRotation, brushAdjusterCenter, GEditorCommon.DefaultRadialFloatFieldStyle, 1f);
            }
            if (showOpacityAdjuster)
            {
                painter.BrushOpacity = GEditorCommon.RadialFloatField("Opacity", painter.BrushOpacity, brushAdjusterCenter, GEditorCommon.DefaultRadialFloatFieldStyle, 0.01f, true);
            }
            if (showScatterAdjuster)
            {
                painter.BrushScatter = GEditorCommon.RadialFloatField("Scatter", painter.BrushScatter, brushAdjusterCenter, GEditorCommon.DefaultRadialFloatFieldStyle, 0.01f, true);
            }
        }
        ...
        {
            GEditorSettings.Instance.paintTools.enableSceneViewRadialSelector = EditorGUILayout.ToggleLeft("Press V in Scene View to show selector", GEditorSettings.Instance.paintTools.enableSceneViewRadialSelector, GEditorCommon.ItalicLabel);
