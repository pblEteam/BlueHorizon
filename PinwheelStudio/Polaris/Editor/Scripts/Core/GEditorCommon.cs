...
namespace Pinwheel.Griffin
{
    public static partial class GEditorCommon
    {
        ...
        public static Color selectedItemColor = EditorGUIUtility.isProSkin ? new Color32(62 + 20, 95 + 20, 150 + 20, 255) : new Color32(62, 125, 231, 255);
        ...
        public static Color32 pinwheelPrimaryColor = new Color32(16, 16, 32, 255);
        public static Color32 pinwheelSecondaryColor = new Color32(250, 250, 250, 255);

        public static RectOffset boxOffset = new RectOffset(2, 2, 2, 2);

        private static GUIStyle windowBodyStyle;
        public static GUIStyle WindowBodyStyle
        {
            get
            {
                if (windowBodyStyle == null)
                {
                    windowBodyStyle = new GUIStyle();
                    //windowBodyStyle.margin = new RectOffset(8, 2, 0, 0);
                    windowBodyStyle.padding = new RectOffset(8, 0, 0, 0);
                }
                return windowBodyStyle;
            }
        }
        ...
        public static GUIStyle GrayMiniLabel
        {
            get
            {
                ...
                grayMiniLabel.alignment = TextAnchor.MiddleLeft;
        ...
        private static GUIStyle centeredGrayMiniLabel;
        public static GUIStyle CenteredGrayMiniLabel
        {
            get
            {
                if (centeredGrayMiniLabel == null)
                {
                    centeredGrayMiniLabel = new GUIStyle(EditorStyles.centeredGreyMiniLabel);
                    centeredGrayMiniLabel.alignment = TextAnchor.MiddleCenter;
                    centeredGrayMiniLabel.wordWrap = true;
                }

                return centeredGrayMiniLabel;
            }
        }
        ...
        public static GUIStyle CenteredBoldLabel
        {
            get
            {
                if (centeredBoldLabel == null)
                {
                    ...
                    centeredBoldLabel.richText = true;
                }
        ...
        public static GUIStyle IconButton
        {
            get
            {
                if (iconButton == null)
                {
                    ...
                    iconButton.fixedHeight = 0;
                    iconButton.fixedWidth = 0;
                }
                return iconButton;
            }
        }
        ...
        public static void BrowseFolder(GUIContent label, ref string result, bool allowTyping = false)
        {
            EditorGUILayout.BeginHorizontal();
            //EditorGUILayout.PrefixLabel(label);
            //EditorGUILayout.LabelField(label, result, ItalicLabel, GUILayout.MinWidth(50));
        ...
        public static GUIStyle FoldoutHeaderStyle
        {
            get
            {
                ...
                {
                    foldoutHeaderStyle = new GUIStyle(EditorStyles.boldLabel);
                }
                foldoutHeaderStyle.padding = new RectOffset(-2, 0, 4, 4);
                return foldoutHeaderStyle;
            }
        }

        public static bool Foldout(GUIContent label, bool defaultExpanded, string id, Action innerGUI, GenericMenu context = null, string headerWarning = null)
        {
            EditorGUILayout.GetControlRect(GUILayout.Height(1));
            ...
            Rect caretRect = EditorGUILayout.GetControlRect(GUILayout.Width(1.4f * indentSpace));
            GUI.Label(caretRect, expanded ? caretDown : caretRight, EditorStyles.miniLabel);
            Rect labelRect = EditorGUILayout.GetControlRect();
            GUI.Label(labelRect, label, FoldoutHeaderStyle);
            ...
            if (expanded)
            {
                ...
                DrawBodyBox(bodyBoxOffset.Add(bodyRect), false);
            }
        ...
        public static bool Foldout(string label, bool defaultExpanded, string id, Action innerGUI, GenericMenu context = null, string headerWarning = null)
        {
            ...
            return Foldout(labelContent, defaultExpanded, id, innerGUI, context, headerWarning);
        }
        ...
        public static void DrawLinks(string text, params string[] urls)
        {
            if (GEditorSettings.Instance.general.enableAffiliateLinks)
            {
                Rect r = EditorGUILayout.GetControlRect();
        ...
        public class RadialSelectorStyles
        {
            public float outerRadius;
            public float innerRadius;
            public float iconSize;

            public float outerRadiusLarge;
            ...
            public GUIStyle itemLabelStyle;
            public GUIStyle itemDescriptionStyle;
        }

        private static RadialSelectorStyles defaultRadialSelectorStyle;
        public static RadialSelectorStyles DefaultRadialSelectorStyle
        {
            get
            {
                if (defaultRadialSelectorStyle == null)
                {
                    defaultRadialSelectorStyle = new RadialSelectorStyles()
                    {
                        innerRadius = 75,
                        outerRadius = 150,
                        iconSize = 24,

                        outerRadiusLarge = 160,
                        iconSizeLarge = 36,

                        innerBackgroundColor = new Color32(45, 45, 45, 255),
                        outerBackgroundColor = new Color32(64, 64, 64, 210),
                        selectedItemBackgroundColor = selectedItemColor
                    };

                    GUIStyle itemLabelStyle = new GUIStyle(EditorStyles.label);
                    itemLabelStyle.fontStyle = FontStyle.Bold;
                    itemLabelStyle.fontSize = 18;
                    itemLabelStyle.alignment = TextAnchor.MiddleCenter;
                    itemLabelStyle.wordWrap = true;
                    defaultRadialSelectorStyle.itemLabelStyle = itemLabelStyle;

                    GUIStyle itemDescriptionStyle = new GUIStyle(EditorStyles.label);
                    itemDescriptionStyle.alignment = TextAnchor.UpperCenter;
                    itemDescriptionStyle.wordWrap = true;
                    defaultRadialSelectorStyle.itemDescriptionStyle = itemDescriptionStyle;
                }
                return defaultRadialSelectorStyle;
            }
        }

        public static T RadialSelectorEnum<T>(T value, Vector2 center, IRadialSelectorProvider<T> provider, RadialSelectorStyles style) where T : System.Enum
        {
            T[] enumValue = provider.GetItemValues(null);
            int itemCount = enumValue.Length;
            float dRad = Mathf.PI * 2f / itemCount;
            T selectedValue = value;

            if (Vector2.Distance(Event.current.mousePosition, center) > style.innerRadius)
            {
                Vector2 mouseRay = (Event.current.mousePosition - center).normalized;
                Matrix4x4 rayRotation = Matrix4x4.Rotate(Quaternion.FromToRotation(Vector3.right, Vector3.up));
                mouseRay = rayRotation.MultiplyVector(mouseRay);

                float mouseRad = GUtilities.GetAngleFromRay(mouseRay);
                float fIndex = mouseRad / dRad;
                int selectedItemIndex = Mathf.FloorToInt(fIndex);
                selectedValue = enumValue[selectedItemIndex];
            }

            Handles.BeginGUI();
            Handles.color = style.outerBackgroundColor;
            Handles.DrawSolidDisc(center, Vector3.forward, style.outerRadius);
            for (int i = 0; i < itemCount; ++i)
            {
                bool isSelectedValue = selectedValue.Equals(enumValue[i]);
                float outerRadius = isSelectedValue ? style.outerRadiusLarge : style.outerRadius;
                float rad = 1.5f * Mathf.PI + (i + 0.5f) * dRad;
                float r = (style.innerRadius + outerRadius) * 0.5f;
                Vector2 p0 = new Vector2(r * Mathf.Cos(rad), r * Mathf.Sin(rad));
                Vector2 p = center + p0;
                Handles.color = new Color(1, 0, 0, 1 - i * 1.0f / itemCount);

                if (isSelectedValue)
                {
                    Handles.color = style.selectedItemBackgroundColor;
                    float arcStartRad = 1.5f * Mathf.PI + i * dRad;
                    Vector2 arcStart = new Vector2(Mathf.Cos(arcStartRad), Mathf.Sin(arcStartRad));
                    Handles.DrawSolidArc(center, Vector3.forward, arcStart, dRad * Mathf.Rad2Deg, style.outerRadiusLarge);
                }

                Texture2D icon = provider.GetIcon(enumValue[i]);
                float iconSize = isSelectedValue ? style.iconSizeLarge : style.iconSize;
                Rect iconRect = new Rect() { size = Vector2.one * iconSize, center = p };
                GUI.DrawTexture(iconRect, icon);
            }


            Handles.color = style.innerBackgroundColor;
            Handles.DrawSolidDisc(center, Vector3.forward, style.innerRadius);

            Rect innerRect = new Rect() { size = Vector2.one * style.innerRadius * 1.4f, center = center };
            Rect labelRect = new Rect(innerRect.x, innerRect.y, innerRect.width, innerRect.height * 0.5f);
            GUI.Label(labelRect, provider.GetItemLabel(selectedValue), style.itemLabelStyle);

            Rect descriptionRect = new Rect(innerRect.x, innerRect.center.y, innerRect.width, innerRect.height * 0.5f);
            GUI.Label(descriptionRect, provider.GetItemDescription(selectedValue), style.itemDescriptionStyle);

            Handles.EndGUI();
            return selectedValue;
        }

        class GSplatPrototypeRadialSelectorProvider : IRadialSelectorProvider<GSplatPrototype>
        {
            public GSplatPrototype[] GetItemValues(object context = null)
            {
                List<GSplatPrototype> prototypes = context as List<GSplatPrototype>;
                return prototypes.ToArray();
            }
            public Texture2D GetIcon(GSplatPrototype value)
            {
                return value.Texture ?? Texture2D.blackTexture;
            }

            public string GetItemLabel(GSplatPrototype value)
            {
                return "Splat Texture";
            }

            public string GetItemDescription(GSplatPrototype value)
            {
        ...
        public static int RadialSelectorSplatPrototype(int selectedIndex, List<GSplatPrototype> prototypes, Vector2 center, RadialSelectorStyles style)
        {
            GSplatPrototypeRadialSelectorProvider provider = new GSplatPrototypeRadialSelectorProvider();
            selectedIndex = RadialSelectorObject<GSplatPrototype>(selectedIndex, prototypes, center, provider, style);
            return selectedIndex;
        }

        class GTreePrototypeRadialSelectorProvider : IRadialSelectorProvider<GTreePrototype>
        {
            public GTreePrototype[] GetItemValues(object context = null)
            {
                List<GTreePrototype> prototypes = context as List<GTreePrototype>;
                return prototypes.ToArray();
            }
            public Texture2D GetIcon(GTreePrototype value)
            {
                if (value.Prefab != null)
                {
                    return AssetPreview.GetAssetPreview(value.Prefab);
                }
                else
                {
            ...
            public string GetItemLabel(GTreePrototype value)
            {
                return "Tree";
            }

            public string GetItemDescription(GTreePrototype value)
            {
                if (value.Prefab != null)
                {
        ...
        public static int RadialSelectorTreePrototype(int selectedIndex, List<GTreePrototype> prototypes, Vector2 center, RadialSelectorStyles style)
        {
            GTreePrototypeRadialSelectorProvider provider = new GTreePrototypeRadialSelectorProvider();
            selectedIndex = RadialSelectorObject<GTreePrototype>(selectedIndex, prototypes, center, provider, style);
            return selectedIndex;
        }

        private static int RadialSelectorObject<T>(int selectedIndex, List<T> prototypes, Vector2 center, IRadialSelectorProvider<T> provider, RadialSelectorStyles style)
        {
            int itemCount = prototypes.Count;
            float dRad = Mathf.PI * 2f / itemCount;
            ...
            if (Vector2.Distance(Event.current.mousePosition, center) > style.innerRadius)
            {
                ...
                float mouseRad = GUtilities.GetAngleFromRay(mouseRay);
                float fIndex = mouseRad / dRad;
                selectedIndex = Mathf.FloorToInt(fIndex);
            }
            ...
            for (int i = 0; i < itemCount; ++i)
            {
                bool isSelectedValue = selectedIndex == i;
                float outerRadius = isSelectedValue ? style.outerRadiusLarge : style.outerRadius;
                float rad = 1.5f * Mathf.PI + (i + 0.5f) * dRad;
                float r = (style.innerRadius + outerRadius) * 0.5f;
                Vector2 p0 = new Vector2(r * Mathf.Cos(rad), r * Mathf.Sin(rad));
                ...
                Texture2D icon = provider.GetIcon(prototypes[i]);
                float iconSize = isSelectedValue ? style.iconSizeLarge : style.iconSize;
                Rect iconRect = new Rect() { size = Vector2.one * iconSize, center = p };
                GUI.DrawTexture(iconRect, icon);
            }
            ...
            Rect innerRect = new Rect() { size = Vector2.one * style.innerRadius * 1.4f, center = center };
            Rect labelRect = new Rect(innerRect.x, innerRect.y, innerRect.width, innerRect.height * 0.5f);
            GUI.Label(labelRect, provider.GetItemLabel(prototypes[selectedIndex]), style.itemLabelStyle);

            Rect descriptionRect = new Rect(innerRect.x, innerRect.center.y, innerRect.width, innerRect.height * 0.5f);
            GUI.Label(descriptionRect, provider.GetItemDescription(prototypes[selectedIndex]), style.itemDescriptionStyle);

            Handles.EndGUI();
            return selectedIndex;
        }

        public class RadialFloatFieldStyle
        {
            public float radius;
        ...
        private static RadialFloatFieldStyle defaultRadialFloatFieldStyle;
        public static RadialFloatFieldStyle DefaultRadialFloatFieldStyle
        {
            get
            {
                if (defaultRadialFloatFieldStyle == null)
                {
                    defaultRadialFloatFieldStyle = new RadialFloatFieldStyle()
                    {
                        radius = 75,
                        backgroundColor = new Color32(64, 64, 64, 210)
                    };
                    ...
                    defaultRadialFloatFieldStyle.labelStyle = labelStyle;

                    GUIStyle valueStyle = new GUIStyle(EditorStyles.label);
                    valueStyle.fontSize = 25;
                    valueStyle.alignment = TextAnchor.UpperCenter;
                    valueStyle.wordWrap = true;
                    defaultRadialFloatFieldStyle.valueStyle = valueStyle;
                }
                return defaultRadialFloatFieldStyle;
            }
        }

        public static float RadialFloatField(string label, float value, Vector2 center, RadialFloatFieldStyle style, float step = 1, bool showAsPercentage = false)
        {
            ...
            Handles.BeginGUI();
            Handles.color = style.backgroundColor;
            Handles.DrawSolidDisc(center, Vector3.forward, style.radius);

            Rect rect = new Rect() { size = Vector2.one * style.radius * 1.4f, center = center };
            Rect labelRect = new Rect(rect.x, rect.y, rect.width, rect.height * 0.5f);
            GUI.Label(labelRect, label, style.labelStyle);

            string valueString = showAsPercentage ? ((value * 100f).ToString("0") + "%") : value.ToString("0.00");
            Rect valueRect = new Rect(rect.x, rect.center.y, rect.width, rect.height * 0.5f);
            GUI.Label(valueRect, valueString, style.valueStyle);
            Handles.EndGUI();
            return value;
        }
