...
namespace Pinwheel.Griffin
{
    public static class GPackageInitializer
    {
        ...
        public static string GRIFFIN_3_KW = "GRIFFIN_3";
...
#pragma warning disable 0414
        ...
        public static bool isCollectionsInstalled = false;
#pragma warning restore 0414
...
#if GRIFFIN 
            EditorCoroutineUtility.StartCoroutineOwnerless(IPostInitTask());
#endif
        }


#if GRIFFIN 
        private static IEnumerator IPostInitTask()
        {
            yield return new WaitForSeconds(1);
            Pinwheel.Griffin.Wizard.GWizardEditorCommon.CheckForUrpFirstTimeImport();            
            if (!GUpdateChecker.CheckedToday())
            {
                GUpdateChecker.CheckForUpdate();
            }
        }
#endif
        ...
        private static void OnPackageListed()
        {
            ...
            else
            {
                foreach (UnityEditor.PackageManager.PackageInfo p in listPackageRequest.Result)
                {
                    ...
                    if (p.name.Equals("com.unity.collections"))
                    {
                        isCollectionsInstalled = true;
                    }
                }
            }
            ...
            else if (!isCollectionsInstalled)
            {
                addPackageRequest = Client.Add("com.unity.collections");
                EditorApplication.update += OnPackageAdded;
                EditorApplication.QueuePlayerLoopUpdate();
            }
        ...
        private static void SetupKeywords()
        {
            ...
            isDirty = isDirty || SetKeywordActive(symbolList, GRIFFIN_3_KW, IsAllDependenciesInstalled());
        ...
        private static bool SetKeywordActive(List<string> kwList, string keyword, bool active)
        {
            ...
            if (active && !kwList.Contains(keyword))
            {
                kwList.Add(keyword);
                isDirty = true;
                Debug.Log($"POLARIS: Adding {keyword} to project Scripting Symbols.");
            }
            else if (!active && kwList.Contains(keyword))
            {
                kwList.RemoveAll(s => s.Equals(keyword));
                Debug.Log($"POLARIS: Removing {keyword} from project Scripting Symbols.");
                isDirty = true;
            }
            return isDirty;
        }
