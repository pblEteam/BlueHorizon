...
namespace Pinwheel.Griffin.BackupTool
{
    public static class GBackup
    {
        ...
        public static void BackupTerrain(GStylizedTerrain t, string backupName, List<GTerrainResourceFlag> flags)
        {
            ...
            if (flags.Contains(GTerrainResourceFlag.StreamMap))
            {
                BackupStreamMap(t, backupName);
            }
#endif
...
#if __MICROSPLAT_POLARIS__ && __MICROSPLAT_STREAMS__
        private static void BackupStreamMap(GStylizedTerrain t, string backupName)
        {
            Color32[] streamMapData = t.TerrainData.Shading.StreamMap.GetPixels32();
            byte[] dataR = new byte[streamMapData.Length];
            byte[] dataG = new byte[streamMapData.Length];
            byte[] dataB = new byte[streamMapData.Length];
            byte[] dataA = new byte[streamMapData.Length];
            for (int i = 0; i < streamMapData.Length; ++i)
            {
                dataR[i] = streamMapData[i].r;
                dataG[i] = streamMapData[i].g;
                dataB[i] = streamMapData[i].b;
                dataA[i] = streamMapData[i].a;
            }
            ...
            GBackupFile.Create(
                backupName,
                string.Format("{0}_{1}", t.TerrainData.Id, GBackupFile.STREAM_R_SUFFIX),
                dataR);
            GBackupFile.Create(
                backupName,
                string.Format("{0}_{1}", t.TerrainData.Id, GBackupFile.STREAM_G_SUFFIX),
                dataG);
            GBackupFile.Create(
                backupName,
                string.Format("{0}_{1}", t.TerrainData.Id, GBackupFile.STREAM_B_SUFFIX),
                dataB);
            GBackupFile.Create(
                backupName,
                string.Format("{0}_{1}", t.TerrainData.Id, GBackupFile.STREAM_A_SUFFIX),
                dataA);
        }
#endif
...
#if __MICROSPLAT_POLARIS__ && __MICROSPLAT_STREAMS__
        private static void RestoreStreamMap(GStylizedTerrain t, string backupName)
        {
            string rFileName = string.Format("{0}_{1}", t.TerrainData.Id, GBackupFile.STREAM_R_SUFFIX);
            string gFileName = string.Format("{0}_{1}", t.TerrainData.Id, GBackupFile.STREAM_G_SUFFIX);
            string bFileName = string.Format("{0}_{1}", t.TerrainData.Id, GBackupFile.STREAM_B_SUFFIX);
            string aFileName = string.Format("{0}_{1}", t.TerrainData.Id, GBackupFile.STREAM_A_SUFFIX);
            ...
            int resolution = Mathf.RoundToInt(Mathf.Sqrt(mapColors.LongLength));
            t.TerrainData.Shading.StreamMapResolution = resolution;
            t.TerrainData.Shading.StreamMap.SetPixels32(mapColors);
            t.TerrainData.Shading.StreamMap.Apply();
        }
#endif
