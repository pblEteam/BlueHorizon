...
namespace Pinwheel.Griffin
{
    ...
    public static class GUtilities
    {
        ...
        public static Transform GetOrCreateChildTransform(Transform parent, string name)
        {
        ...
        public static Rect GetLeftDirtyRect()
        {
        ...
        public static Rect GetTopDirtyRect()
        {
        ...
        public static Rect GetRightDirtyRect()
        {
            Rect topRect = new Rect();
            topRect.size = new Vector2(0.01f, 1f);
            topRect.center = new Vector2(1, 0.5f);
            return topRect;
        }

        public static Rect GetBottomDirtyRect()
        {
            Rect topRect = new Rect();
            topRect.size = new Vector2(1, 0.01f);
            topRect.center = new Vector2(0.5f, 0);
            return topRect;
        }

        public static Rect GetLeftTopDirtyRect()
        {
            Rect leftRect = new Rect();
            leftRect.size = new Vector2(0.01f, 0.01f);
            leftRect.center = new Vector2(0, 1f);
            return leftRect;
        }

        public static Rect GetTopRightDirtyRect()
        {
            Rect leftRect = new Rect();
            leftRect.size = new Vector2(0.01f, 0.01f);
            leftRect.center = new Vector2(1f, 1f);
            return leftRect;
        }

        public static Rect GetRightBottomDirtyRect()
        {
            Rect leftRect = new Rect();
            leftRect.size = new Vector2(0.01f, 0.01f);
            leftRect.center = new Vector2(1f, 0f);
            return leftRect;
        }

        public static Rect GetBottomLeftDirtyRect()
        {
            Rect leftRect = new Rect();
            leftRect.size = new Vector2(0.01f, 0.01f);
            leftRect.center = new Vector2(0f, 0f);
            return leftRect;
        }

        public static Color MatchHeightMapSample(int count, params Color[] samples)
        {
            float avgHeight = 0;
            ...
            {
                Color s = samples[i];
                avgHeight += GCommon.DecodeTerrainHeight(new Vector2(s.r, s.g));
                avg += s;
            }
            avgHeight = avgHeight / count;
            Vector2 enc = GCommon.EncodeTerrainHeight(avgHeight);
            avg = avg / count;
            avg.r = enc.x;
            avg.g = enc.y;
            return avg;
        ...
        public static float GetAngleFromRay(Vector2 ray)
        {
            if (ray.y >= 0)
            {
                return Mathf.Atan2(ray.y, ray.x);
            }
            else
            {
                return Mathf.PI + Mathf.Atan2(-ray.y, -ray.x);
            }
        }
    }
}
