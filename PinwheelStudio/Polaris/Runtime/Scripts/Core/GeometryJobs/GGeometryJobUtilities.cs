...
namespace Pinwheel.Griffin
{
    public struct GGeometryJobUtilities
    {
        ...
        public const int VERTEX_MARKER_DIMENSION = 129; //2^(ceil(res/2))+1 vertices on each axis
        ...
        public static float GetFloatBilinear(NativeArray<float> tex, ref Vector2 uv, int texWidth, int texHeight)
        {
            Vector2 pixelCoord = new Vector2(
                Mathf.Lerp(0, texWidth - 1, uv.x),
                Mathf.Lerp(0, texHeight - 1, uv.y));
            //apply a bilinear filter
            int xFloor = Mathf.FloorToInt(pixelCoord.x);
            int xCeil = Mathf.CeilToInt(pixelCoord.x);
            int yFloor = Mathf.FloorToInt(pixelCoord.y);
            ...
            float f00 = tex[To1DIndex(ref xFloor, ref yFloor, ref texWidth)];
            float f01 = tex[To1DIndex(ref xFloor, ref yCeil, ref texWidth)];
            float f10 = tex[To1DIndex(ref xCeil, ref yFloor, ref texHeight)];
            float f11 = tex[To1DIndex(ref xCeil, ref yCeil, ref texHeight)];
