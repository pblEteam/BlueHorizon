...
namespace Pinwheel.Griffin
{
    ...
    public class GTerrainChunk : MonoBehaviour
    {
        ...
        private GTerrainChunkLOD[] ChunkLowerLOD
        {
            get
            {
                if (chunkLowerLOD == null || chunkLowerLOD.Length != Terrain.geometryShortcut.LODCount - 1)
                {
                    chunkLowerLOD = new GTerrainChunkLOD[Terrain.geometryShortcut.LODCount - 1];
        ...
        public Rect GetUvRange()
        {
            ...
            {
                GGeometry g = Terrain.geometryShortcut ? Terrain.geometryShortcut : Terrain.TerrainData.Geometry;
                int gridSize = g.ChunkGridSize;
        ...
        private void RecalculateTangentIfNeeded(Mesh m)
        {
            if (m == null)
                return;
            if (Terrain.shadingShortcut.IsMaterialUseNormalMap())
            {
        ...
        {
            GGeometry geo = Terrain.geometryShortcut ? Terrain.geometryShortcut : Terrain.TerrainData.Geometry;
            if (level <= 0 || level >= geo.LODCount)
                throw new System.IndexOutOfRangeException();
            GTerrainChunkLOD c = ChunkLowerLOD[level - 1];
            if (c == null)
            {
                Transform t = GUtilities.GetOrCreateChildTransform(transform, name + "_LOD" + level);
                GUtilities.ResetTransform(t, transform);
        ...
        {
            GGeometry geo = Terrain.geometryShortcut ? Terrain.geometryShortcut : Terrain.TerrainData.Geometry;
            GTerrainGeneratedData geoData = Terrain.geometryDataShortcut ? Terrain.geometryDataShortcut : Terrain.TerrainData.GeometryData;

            int lodCount = geo.LODCount;
            if (lod < 0 || lod >= lodCount)
            {
                throw new System.ArgumentOutOfRangeException("lod");
            }

            GGeometry.GStorageMode mode = geo.StorageMode;
            if (mode == GGeometry.GStorageMode.SaveToAsset)
            {
                Vector3Int key = GetChunkMeshKey(Index, lod);
                Mesh m = geoData.GetMesh(key);
                if (m == null)
                {
                    m = new Mesh();
                    m.name = GetChunkMeshName(Index, lod);
                    m.MarkDynamic();
                    geoData.SetMesh(key, m);
                }
                return m;
            }
            else
            ...
            GTextureNativeDataDescriptor<Color32>[] hm)
        {
            ...
            Vector3 terrainSize = Terrain.geometryShortcut.Size;
            Rect uvRect = GetUvRange();
            float texelSize = 1.0f / Terrain.geometryShortcut.HeightMapResolution;
        ...
        internal void UpdateMesh(int lod, GAlbedoToVertexColorMode albedoToVertexColor)
        {
            ...
            {
                m.SetVertices(vertexNativeArray);
                m.SetUVs(0, uvsNativeArray);
                m.SetNormals(normalsNativeArray);
                m.SetIndices(trianglesNativeArray, MeshTopology.Triangles, 0);

                if (albedoToVertexColor != GAlbedoToVertexColorMode.None)
                {
                    m.SetColors(vertexColorsNativeArray);
                }
        ...
        internal void SetupLODGroup(int lodCount)
        {
            ...
            bool useLodCurve = GRuntimeSettings.Instance.geometryGeneration.useLodTransitionCurve;
            for (int level = 0; level < lodCount; ++level)
            {
                int i = level;
                float f = (i + 1) * transitionStep;
                float t = 0;
                if (useLodCurve)
                {
                    t = GRuntimeSettings.Instance.geometryGeneration.lodTransition.Evaluate(f);
                }
                else
                {
                    t = 1 - f * f;
                ...
                lods[i] = new LOD(
                    t,
                    new Renderer[] { i == 0 ? MeshRendererComponent : GetChunkLOD(i).MeshRendererComponent });
            }

            LodGroupComponent.SetLODs(lods);

            int maxChildTransformIndex = transform.childCount - 1;
            for (int i = maxChildTransformIndex; i >= lodCount - 1; --i)
            {
                GUtilities.DestroyGameobject(transform.GetChild(i).gameObject);
            }
        }
    }
}
